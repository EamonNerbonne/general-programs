...
namespace SongSearchSite.Code.Handlers
{
    ...
    public class SearchRequestProcessor : IHttpRequestProcessor
    {
        ...
        bool includeRemote, extm3u, coreAttrsOnly, viewXslt, avoidDuplicates;
        OutputFormat format;
        ...
        public PotentialResourceInfo DetermineResource()
        {
            ...
            var searchterms = from pathpart in path.Skip(1).Take(path.Length - 2)
                select pathpart;
            ...
            if (extension == ".m3u" || extension == ".m3u8") {
                context.Response.Headers["Content-Disposition"] = "attachment; filename=" + Uri.EscapeDataString("playlist_" + searchQuery + extension);
                //searchquery has been canonicalized: no dangerous injection possible.
            }

            res.ETag = ResourceInfo.GenerateETagFrom(
                searchQuery,
                includeRemote,
                extm3u,
                format,
        ...
        public void WriteEntireContent()
        {
            ...
            if (extm3u && format == OutputFormat.M3U) {
            ...
            if (coreAttrsOnly) {
                searchResults = searchResults.Where(
                    song => {
                        var mime = SongServeRequestProcessor.guessMIME(song);
                        return mime == SongServeRequestProcessor.MIME_MP3 || mime == SongServeRequestProcessor.MIME_OGG;
                    });
            }
            if (!includeRemote) {
            ...
            if (format == OutputFormat.M3U) {
                ...
                var uriMapper = coreAttrsOnly
                    ? SongDbContainer.LocalSongPathToAppRelativeMapper(context)
                    : SongDbContainer.LocalSongPathToAbsolutePathMapper(context);
                var rootPlaylistElem = new XElement(
                    "songs",
                ...
                if (format == OutputFormat.XML) {
                    ...
                    SearchResultTransform.TransformToHtml(rootPlaylistElem, context.Response.Output);
                }
            }
        }
    ...
    static class SearchResultTransform
    {
        public static readonly XslCompiledTransform SearchTransform = new XslCompiledTransform();

        static SearchResultTransform()
        {
            using (var xsltStream = typeof(SearchResultTransform).Assembly.GetManifestResourceStream("SongSearchSite.searchresult.xsl"))
            using (var xmlReader = XmlReader.Create(xsltStream))
                SearchTransform.Load(xmlReader);
            ;
        }

        public static void TransformToHtml(XElement rootPlaylistElem, TextWriter output)
        {
            using (var dataReader = rootPlaylistElem.CreateReader())

                SearchTransform.Transform(dataReader, null, output);
        }
    }
}
