...
namespace HttpHeaderHelper {
	...
	public class HttpRequestHelper {
		...
		bool canAcceptRangeRequest;
		Range[] requestedRanges;
		//		string statusErrorMessageBody;

		public HttpContext Context { get { return context; } }

		bool AssertMethodIsOneOf(params HttpMethod[] acceptedMethods) {
		...
		void SetFinalStatus(int statusCode, string statusDescription = null, string statusLongMessage = null) {
		...
		void SetFinalStatus(ResourceError error) {
		...
		public HttpRequestHelper(HttpContext httpContext) {
			context = httpContext;
			method = DetermineHttpMethod();
			requestFinished = false;
		}

		public void Process(IHttpRequestProcessor reqProc_arg) {
			reqProc = reqProc_arg;
		...
		static void IgnoreDisconnectionExceptions(Action a) {
			try {
				a();
				//why are there so many exceptions for essentially the same thing?
		...
		private void Step7aPerformRangeRequests() {
			...
			if (method == HttpMethod.GET) {
				IgnoreDisconnectionExceptions(() => reqProc.WriteByteRange(requestedRanges[0]));
		...
		private void Step7bPerformNormalRequests() {
			...
			if (method == HttpMethod.GET) {
				IgnoreDisconnectionExceptions(() => reqProc.WriteEntireContent());
			} else if (method == HttpMethod.HEAD) {
				//do nothing
			} else {
				throw new Exception("Unsupported http method:" + method);
