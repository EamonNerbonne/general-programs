...
namespace RealSimilarityMds
{
    class Program
    {
        ...
        private static CachedDistanceMatrix LoadCachedMatrix(LastFmTools tools) {
            NiceTimer timer = new NiceTimer();
        ...
        private static double[,] DoMds(CachedDistanceMatrix cachedMatrix) {
            Random r = new Random();
            int dimensions=5;
            ...
            using (Hitmds mdsImpl = new Hitmds(cachedMatrix.Mapping.Count, dimensions, (i, j) => cachedMatrix.Matrix[i, j].IsFinite()?cachedMatrix.Matrix[i, j]:maxDist*10, r)) {
                mdsImpl.mds_train(cachedMatrix.Mapping.Count * 20, 2.0, 0.0, (cycle, ofTotal, src) => { });
