...
namespace SongDataLib
{
    ...
    public class SongFileData : MinimalSongFileData
    {
        ...
        {
            if (fileObj.Extension == ".opus") {
                var md = new ATL.Track(fileObj.FullName);
                title = md.Title;
                artist = md.Artist ?? md.Composer;
                ...
                bitrate = md.Bitrate;
                length = md.Duration;
                rating = md.Rating == 0 ? default(int?) : md.Rating;
            } else {
            ...
            popularity = popEst?.EstimatePopularity(artist, title) ?? default(Popularity);
        }
        ...
        static IEnumerable<TagLib.Id3v2.UserTextInformationFrame> UserTextInformationFrames(File file)
        {
        ...
        static readonly XName songN = "song",
            titleN = "title",
            artistN = "artist",
            performerN = "performer",
            composerN = "composer",
            albumN = "album",
            commentN = "comment",
            genreN = "genre",
            yearN = "year",
            trackN = "track",
            trackcountN = "trackcount",
            bitrateN = "bitrate",
            lengthN = "length",
            samplerateN = "samplerate",
            channelsN = "channels",
            lastmodifiedTicksN = "lastmodifiedTicks",
            ratingN = "rating",
            artistpopularityN = "popA",
            titlepopularityN = "popT",
            trackGainN = "Tgain",
            filesizeN = "filesize";
            ...
            : base(baseUri, from, isLocal)
        {
            ...
            if (lastmodifiedTicks.HasValue) {
                LastWriteTimeUtc = new DateTime(lastmodifiedTicks.Value, DateTimeKind.Utc);
            }
        ...
        { //only yield "distinctive" search values
            get
            {
        ...
        public override IEnumerable<SongRef> PossibleSongs
        {
            get { return artist != null && title != null ? Enumerable.Repeat(SongRef.Create(artist, title), 1) : base.PossibleSongs; }
        }
    }
}
