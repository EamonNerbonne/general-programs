...
namespace SongDataLib
{
    ...
    public struct SortOrdering : IEquatable<SortOrdering>
    {
        ...
        public static SortOrdering FromSeq(IEnumerable<int> cols)
        {
            var set = new HashSet<int>();
            return new SortOrdering(cols.Where(i => i != 0 && set.Add(Math.Abs(i))).ToArray());
        }

        public static SortOrdering FromString(string rep) { return FromSeq((rep ?? "").Split(splitBy, StringSplitOptions.RemoveEmptyEntries).Select(ParseOneOrderCol)); }
    ...
    public static class SortOrder
    {
        ...
        public static int[] RankMapFor(ISongFileData[] songs, SortOrdering ordering)
        {
            ...
            foreach (var column in ordering.Order) {
                switch (column.Item1) {
                    case SongColumn.Album:
                        indexes = indexes.StringOrder(i => !(songs[i] is SongFileData) ? null : ((SongFileData)songs[i]).album, column.Item2);
                        break;
                    case SongColumn.Artist:
                        indexes = indexes.StringOrder(i => !(songs[i] is SongFileData) ? null : ((SongFileData)songs[i]).artist, column.Item2);
                        break;
                    case SongColumn.Rating:
                        indexes =
                            indexes.OrderByEither(
                                i =>
                                    ((!(songs[i] is SongFileData) ? null : ((SongFileData)songs[i]).rating) ?? 2.5)
                                        * (!(songs[i] is SongFileData) ? 0 : ((SongFileData)songs[i]).popularity.TitlePopularity),
                                !column.Item2);
                        break;
                    case SongColumn.Time:
                        indexes = indexes.OrderByEither(i => !(songs[i] is SongFileData) ? 0 : ((SongFileData)songs[i]).length, column.Item2);
                        break;
                    case SongColumn.Title:
                        indexes = indexes.StringOrder(i => !(songs[i] is SongFileData) ? null : ((SongFileData)songs[i]).title, column.Item2);
                        break;
                    case SongColumn.TrackNumber:
                        indexes = indexes.OrderByEither(i => !(songs[i] is SongFileData) ? 0 : ((SongFileData)songs[i]).track, column.Item2);
                        break;
                    default:
