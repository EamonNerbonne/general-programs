var HtmlFactory23 = (function (D) {
	...
	function appendArrayIntoEmpty(node, arr, i) {//can't check for window.Node: not available in IE8.
		var len = arr.length, acc_str = '';
		while (i < len) {
			var argVal = arr[i++];
			if (argVal instanceof Node) {
				if (acc_str.length > 0) {
					node.textContent = acc_str;
				}
				node.appendChild(argVal);
				appendArrayInto(node, arr, i);
				return;
			}
			else if (isArray(argVal)) {
				if (acc_str.length > 0) {
	...
	function fillNode(node) {
		return function (attrs) {
			...
			var len = arguments.length, acc_str = '',i=1;
			while (i < len) {
				...
				if (argVal instanceof Node) {
					...
					node.appendChild(argVal);
					appendArrayInto(node, arguments, i);
					return node;
				}
				else if (isArray(argVal)) {
					if (acc_str.length > 0) {
	...
	function mkElem(name) {
		return function (attrs) {
			...
			appendArrayIntoEmpty(node, arguments, 1);
			return node;
		}
	}
