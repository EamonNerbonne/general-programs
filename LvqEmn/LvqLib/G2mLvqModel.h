...
public:
	...
	inline void learnFromImpl(MatrixBase<MatrixType> const & newPoint, int classLabel);
	void ClassBoundaryDiagramImpl(double x0, double x1, double y0, double y1, MatrixXi & classDiagram) const;
};

template<typename MatrixType>
inline void G2mLvqModel::learnFromImpl(MatrixBase<MatrixType> const & trainPoint, int trainLabel) {
	using namespace std;
