...
namespace LvqGui.LvqPlotting
{
    sealed class LvqStatName
    {
        ...
        LvqStatName(string compoundName, int index)
        {
            ...
            Index = index;
            var splitName = compoundName.Split('!');
            if (splitName.Length < 2) {
                throw new ArgumentException("compound name has too few components");
            }

            if (splitName.Length > 3) {
                throw new ArgumentException("compound name has too many components");
            }
        ...
        public static LvqStatName Create(string compoundName, int index)
            => new(compoundName, index);
    }
}
