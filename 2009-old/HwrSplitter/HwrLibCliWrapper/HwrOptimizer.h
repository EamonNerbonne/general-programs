...
namespace HwrLibCliWrapper {
	public ref class HwrOptimizer {
		...
		initonly HwrDataModel::SymbolClasses^ managedSymbols;
	public:
		HwrOptimizer(HwrDataModel::SymbolClasses^ symbolClasses)
			: nativeSymbols(gcnew SymbolLearningData(symbolClasses->Count )) 
		...
		bool SplitWords(ImageStruct<signed char> block, int cropXoffset, HwrDataModel::HwrTextLine^ textLine, SymbolLearningData ^ dataSink);
